        ...
        <div class="scroll-area">
            ...
            <section id="dashboard" class="fade-in">
                ...
                <div style="margin-bottom: 24px; display: flex; justify-content: flex-end;">
                    <a href="http://10.147.17.116:8000/info.php" target="_blank" class="btn-primary">
                        📚 Learn More
                    </a>
                </div>
                ...
                <div class="card bg-dark" style="background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%); color: white;">
                    ...
                    <div style="display: flex; gap: 12px; flex-wrap: wrap;">
                        <button onclick="exportData('volunteers')" class="btn-primary"><i class="fas fa-download"></i> Volunteers</button>
                        <button onclick="exportData('resources')" class="btn-primary"><i class="fas fa-download"></i> Inventory</button>
                        <button onclick="exportData('distributions')" class="btn-primary"><i class="fas fa-download"></i> Logistics</button>
                        <button onclick="exportData('disasters')" class="btn-primary"><i class="fas fa-download"></i> Disasters</button>
                    </div>
                </div>
            </section>
    ...
    <script>
        ...
        async function fetchSession() {
            try {
                ...
                // --- NEW SESSION AUTH CHECK ---
                if (!response.ok) throw new Error('Session fetch failed');
                const sessionData = await response.json();

                // If authentication fails (e.g. from your PHP logic: is_api_authenticated = false)
                // You can add logic here if your API returns something like { authenticated: false }
                // For now, we update the UI
                ...
                if(sessionData.user_name || sessionData.FullName) {
                    ...
                    // Fallback to Guest/Redirect logic if needed
                    // window.location.href = "http://10.147.17.30:8000/login.php"; 
                }
